...
def send_onesignal_from_log(doc, method):
	...
	# Case 2: Notification to a role
		...
		role_users = frappe.get_all("Has Role", filters={"role": doc.for_role}, fields=["parent as user"])
		recipients = [r.user for r in role_users if r.user and r.user not in ("Administrator", "Guest")]

	# Case 3: Notification to everyone
	else:
		all_users = frappe.get_all("User", filters={"enabled": 1}, pluck="name")
		recipients = [u for u in all_users if u not in ("Administrator", "Guest")]
